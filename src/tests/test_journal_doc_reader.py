...
def test__pickle_df(journal_document_reader):
    # Create test data and pickle it, then check that the pickled data is identical to the data in memory and that no unpickled file exists.
    journal_document_reader = JournalDocumentReader("../../data/raw/papers-pkl/",
                                                    "../../data/test_assets/pickled_cleaned_test_data/")
    journal_document_reader._validate_raw_data()
    journal_document_reader._filter_duplicate_documents()
    journal_document_reader._decode_text()
    journal_document_reader._tokenize_text()
