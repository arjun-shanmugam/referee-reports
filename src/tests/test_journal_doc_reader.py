...
def journal_document_reader():
    return JournalDocumentReader("../../data/raw/papers-pkl/", "../../data/intermediate/")


def test__validate_raw_data():
    journal_document_reader = JournalDocumentReader("../../data/test_assets/papers_with_directory-pkl/",
                                                    "")
...
def test__mwe_retokenize(journal_document_reader):
    tokens = ["The", "coefficient", "on", "\\", "beta", "is", "3", ".", "4", "and", "\\", "gamma", "equals", "2", "."]
    retokenized_list = journal_document_reader._mwe_retokenize(tokens, "\\")
    assert retokenized_list == ["The", "coefficient", "on", "\\beta", "is", "3", ".", "4", "and", "\\gamma", "equals", "2", "."]

    tokens = ["The", "coefficient", "on", "\\", "\\", "beta", "is", "3", ".", "4", "and", "\\", "gamma", "equals", "2", "."]
    retokenized_list = journal_document_reader._mwe_retokenize(tokens, "\\")
    assert journal_document_reader._mwe_retokenize(tokens, "\\") == ["The", "coefficient", "on", "\\\\beta", "is", "3", ".", "4", "and", "\\gamma", "equals",
                                                                     "2", "."]

    tokens = ["The", "coefficient", "on", "\\", "\\", "\\", "beta", "is", "3", ".", "4", "and", "\\", "\\", "gamma", "\\", "equals", "2", ".", "\\"]
    retokenized_list = journal_document_reader._mwe_retokenize(tokens, "\\")
    assert journal_document_reader._mwe_retokenize(tokens, "\\") == ["The", "coefficient", "on", "\\\\\\beta", "is", "3", ".", "4", "and", "\\\\gamma",
                                                                     "\\equals", "2",
                                                                     ".", "\\"]
...
def test__pickle_df(journal_document_reader):
    ...
    journal_document_reader.build_df()

    # Decode the pickled data and store it in a DataFrame.
    decoded_bytes = pkldir.decode(os.path.join(journal_document_reader.path_to_intermediate_data, "papers.txt.pkl"))
    ...
    pd.testing.assert_frame_equal(journal_document_reader.df, decoded_df)
